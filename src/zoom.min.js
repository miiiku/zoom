(function (win, factory) {
  ...
  var domMaskBox, domImgBox, domImgLoading, domPrevBtn, domNextBtn;
  var activeIndex = null, isTransitionEnd = true, isClose = false;
  var container, fullWidth, fullHeight, maxWidth, maxHeight, images = [], unloads = [];
  ...
  const eventScroll = () => {
    if (unloads.length < 1) return window.removeEventListener("scroll", debounce(eventScroll, 400));
  ...
  const eventMouseover = () => {
    domPrevBtn.style.opacity = 1;
    domNextBtn.style.opacity = 1;
  }

  const eventMouseout = () => {
    domPrevBtn.style.opacity = 0;
    domNextBtn.style.opacity = 0;
  }
  ...
  const init = () => {
    ...
    window.addEventListener("resize", debounce(eventResize, 800));
    window.addEventListener("keyup", eventKeyUp);
    eventResize();

    if (lazyLoad && unloads.length > 0) {
      window.addEventListener("scroll", debounce(eventScroll, 400));
      eventScroll();
    }
  }
  ...
  const initCSS = () => {
    let css = `
      ...
      .zoom-btn.prev { left: 30px; }
      .zoom-btn.next { right: 30px; }
    `;
  ...
  const destroy = () => {
    ...
    domImgBox.removeEventListener("mouseover", eventMouseover);
    domImgBox.removeEventListener("mouseout", eventMouseout);
  ...
  const createZoom = () => {
    ...
    domImgBox.addEventListener("mouseover", eventMouseover);
    domImgBox.addEventListener("mouseout", eventMouseout);
